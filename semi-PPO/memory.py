...
class OnPolicyMemory:
    ...
    def append(self, state, action, reward, value, log_prob, dt):

        # print(self._head, self._lim)
    ...
    def compute_values(self, v_last):
        ...
        delta = self._rew_mem[start: idx + 1] + (gamma ** self._dt[start: idx + 1]) * v[1:] - v[:-1]
        #print((gamma ** self._dt[start: idx + 1]))
        ...
        for t in range(idx, start - 1, -1):
            ...
            self._target_v_mem[t] = self._rew_mem[t] + gamma**self._dt[t] * next_R
            self._adv_mem[t] = delta[t - start] + (gamma**self._dt[t - start] * lam) * next_GAE
