...
- $D_k^\ell, D^u_k \in \{0, 1 \}^{|\mathfrak{F}|}$: binary vector that represents the destination of a lot in lower/upper fork of the lifter, which take $\bm{0}$ when lower/upper fork is empty.
...
$$
\mathscr{P}_{p, a} = \begin{cases} 1 & \text{if $a$ brings the lifter to $p$,} \\
0 & \text{otherwise,}
\end{cases} \qquad p \in \mathcal{P}, \quad a \in \mathcal{A}.
$$
