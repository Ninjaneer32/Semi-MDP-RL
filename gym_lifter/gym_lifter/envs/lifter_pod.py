...
class LifterPODEnv(gym.Env):
	def __init__(self, mode):
		# super(gym.Env, self).__init__()
		self.fab = FAB(mode=mode)
		self.num_layers = self.fab.num_layers
		self.state_dim = 4 + 4 * self.fab.num_layers  # pos, lower, upper, to, waiting quantity, waiting time, PODS? ##########4, 4
	...
	def get_obs(self) -> np.ndarray:
		...
		rack_info = [rpos, lower_to / float(self.fab.NUM_FLOOR), upper_to / float(self.fab.NUM_FLOOR), self.is_pod_loaded]
		...
		is_pods = [self.waiting_pod[i] for i in range(len(self.waiting_pod))]

		layer_info = destination + is_pods + waiting_quantity + waiting_time
	...
	def action_map(self, state) -> List[int]:
		...
		wpod = state[4+(3 * NUM_FLOOR) :4+(3 * NUM_FLOOR)+(3 * NUM_FLOOR)]
		#print(len(self.destination), 3*NUM_FLOOR)
		...
		# 	print(lower_floor)
		# 	print(is_pod,"lower")
			# print(self.fab.rack.lower_fork.is_pod)
		# if upper_occupied:
		# 	print(upper_floor)
		# 	print(is_pod,"upper")
			# print(self.fab.rack.upper_fork.is_pod)
	...
	@property
	def waiting_pod(self):
		return self.fab.waiting_pod
