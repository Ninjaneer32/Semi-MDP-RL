...
class FAB:
    ...
    def load_arrival_data(self):
        ...
        dir = str(self.mode) + "F"
        if self.scene==0:
            dir_path = 'assets/POD/'+dir+'/scenario{}/'.format(scenario)
        if self.scene==1:
            dir_path = 'assets/POD/'+dir+'/scenario{}/'.format(scenario)
    ...
    def sim(self, operation: Optional[Tuple[int, int, int]]) -> Dict[str, Any]:
        ...
        if operation is None:
            ...
            floor, pos, low_up, load_unload = operation #(pos, low, up)   (0,1,-1   ,0,1,-1,   )
    ...
    def sim_arrival(self, dt: float):
        ...
        for i in range(begin, self.end):
            ...
            wafer = Wafer(cmd_t=self.data_cmd[i], origin=self.data_from[i], destination=self.data_to[i], pod=self.data_pod[i])
            self.tot+=1
    ...
    @property
    def waiting_pod(self) -> List[int]:
        # pods = np.zeros(self.num_layers)
        # for i, conveyor in enumerate(self.layers.values()):
        #     pods[i] = 0. if conveyors.is_empty else self.t - conveyors.cmd_time
        return [conveyor.is_pod for conveyor in self.layers.values()]
