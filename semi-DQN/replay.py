...
# This code is forked from https://github.com/deepmind/dqn_zoo/blob/master/dqn_zoo/replay.py

from typing import Any, Callable, Generic, List, Mapping, Optional, Sequence, Text, Tuple, TypeVar, NamedTuple
import numpy as np
...
class SumTree:
  ...
  """
    ...
    def resize(self, size: int) -> None:
        """Resize tree, truncating or expanding with zeros as needed."""
...
class PrioritizedDistribution:
    ...
    def __init__(
                 self,
                 capacity: int,
                 priority_exponent: Callable[[int], float],
    ...
    def set_priorities(self, indices: Sequence[int], priorities: Sequence[float], exp) -> None:
        ...
        self._sum_tree.set(indices, _power(priorities, exp))

    def update_priorities(self, indices: Sequence[int],
                          priorities: Sequence[float], exp) -> None:
    ...
    def sample(self, size: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        uniform_prob = 1. / self.size  # np.asarray is for pytype.
...
class PrioritizedTransitionReplay(Generic[ReplayStructure]):
    ...
    def add(self, item: ReplayStructure, priority: float) -> None:
        ...
        self._distribution.set_priorities([index], [priority], self.priority_exponent)
    ...
    def sample(self, size: int) -> Tuple[Any, np.ndarray, np.ndarray]:
    ...
    def update_priorities(self, indices: Sequence[int], priorities: Sequence[float]) -> None:
        ...
        self._distribution.update_priorities(indices, priorities, self.priority_exponent)
    ...
    @property
    def priority_exponent(self):
        """Importance sampling exponent at current step."""
        return self._priority_exponent(self._t)
