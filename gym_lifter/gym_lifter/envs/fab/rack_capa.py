...
class Rack:
    def __init__(self):
        ...
        self.upper_fork_: Optional[Wafer] = None
        self.lower_fork_: Optional[Wafer] = None

        self.is_upper_loaded: bool = False
        self.is_lower_loaded: bool = False
        self.is_upper_loaded_: bool = False
        self.is_lower_loaded_: bool = False
    ...
    def release_upper_fork(self) -> Wafer:
        ...
        return wafer_released
    ####################
    def release_lower_fork_(self) -> Wafer:
        assert self.is_lower_loaded_
        wafer_released = deepcopy(self.lower_fork_)
        self.lower_fork_ = None
        self.is_lower_loaded_ = False
        return wafer_released

    def release_upper_fork_(self) -> Wafer:
        assert self.is_upper_loaded_
        wafer_released = deepcopy(self.upper_fork_)
        self.upper_fork_ = None
        self.is_upper_loaded_ = False
        return wafer_released

    def load_lower_(self, wafer: Wafer):
        assert not self.is_lower_loaded_
        self.lower_fork_ = wafer
        self.is_lower_loaded_ = True
        return

    def load_upper_(self, wafer: Wafer):
        assert not self.is_upper_loaded_
        self.upper_fork_ = wafer
        self.is_upper_loaded_ = True
        return
    ...
    @property
    def destination_(self) -> Tuple[int, int]:
        destination1 = self.lower_fork_.destination if self.is_lower_loaded_ else 0
        destination2 = self.upper_fork_.destination if self.is_upper_loaded_ else 0
        return destination1, destination2
